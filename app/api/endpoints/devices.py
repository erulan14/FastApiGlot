...
@router.post('/devices/',
             response_model=DeviceInCreate,
             tags=["devices"],
             dependencies=[Depends(JWTBearer())]
)
async def create_new_device(device: DeviceInCreate = Body(..., embed=True),
                            db: AsyncIOMotorClient = Depends(get_database)
                            ):
    db_device = await create_devices_db(db, device)
    return Response(
        json.dumps(db_device.dict()),
...
async def update_device(db: AsyncIOMotorClient = Depends(get_database),
                        id: str | None = None
):
